...
class resnet(_FPN):
    ...
    def _init_modules(self):
        ...
        #for p in self.RCNN_latlayer1.parameters(): p.requires_grad = False
        #for p in self.RCNN_latlayer2.parameters(): p.requires_grad = False
        #for p in self.RCNN_latlayer3.parameters(): p.requires_grad = False

        #for p in self.RCNN_smooth1.parameters(): p.requires_grad = False
        #for p in self.RCNN_smooth2.parameters(): p.requires_grad = False
        #for p in self.RCNN_smooth3.parameters(): p.requires_grad = False

        for p in self.RCNN_layer4.parameters(): p.requires_grad = False
    ...
    def train(self, mode=True):
        ...
        if mode:
            ...
            self.RCNN_layer3.eval()
            #self.RCNN_layer4.train()
            self.RCNN_layer4.eval()

            self.RCNN_smooth1.train()
            #self.RCNN_smooth1.eval()
            self.RCNN_smooth2.train()
            #self.RCNN_smooth2.eval()
            self.RCNN_smooth3.train()
            #self.RCNN_smooth3.eval()

            self.RCNN_latlayer1.train()
            #self.RCNN_latlayer1.eval()
            self.RCNN_latlayer2.train()
            #self.RCNN_latlayer2.eval()
            self.RCNN_latlayer3.train()
            #self.RCNN_latlayer3.eval()
            self.RCNN_toplayer.train()
            #self.RCNN_toplayer.eval()
