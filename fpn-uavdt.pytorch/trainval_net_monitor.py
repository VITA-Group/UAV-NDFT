...
if __name__ == '__main__':
    ...
    if args.dataset == "visdrone":
        ...
        args.set_cfgs = ['FPN_ANCHOR_SCALES', '[16, 32, 64, 128, 256]', 'FPN_FEAT_STRIDES', '[4, 8, 16, 32, 64]',
                         'ANCHOR_RATIOS', '[0.5, 1, 2]', 'MAX_NUM_GT_BOXES', '60']
    elif args.dataset == "uav":
        args.imdb_name = "uav_2017_trainval"
        args.imdbval_name = "uav_2017_test"
        args.set_cfgs = ['FPN_ANCHOR_SCALES', '[16, 32, 64, 128, 256]', 'FPN_FEAT_STRIDES', '[4, 8, 16, 32, 64]',
                         'ANCHOR_RATIOS', '[0.5, 1, 2]', 'MAX_NUM_GT_BOXES', '107']
    ...
    for key, value in dict(FPN.named_parameters()).items():
        ...
        if value.requires_grad and not any(name in key for name in ['RCNN_weather', 'RCNN_altitude', 'RCNN_angle',
                                                                    'RCNN_weather_score', 'RCNN_altitude_score',
                                                                    'RCNN_angle_score']):
    ...
    if args.resume:
        load_name = os.path.join(model_dir, 'fpn_{}_{}_{}_adv.pth'.format(args.checksession, args.checkepoch,
                                                                       args.checkpoint))
        ...
        load_name = os.path.join(os.path.join(args.model_dir, 'baseline'),
    ...
    while niter < iters_per_epoch * ((args.max_epochs - args.start_epoch) + 1):
            ...
            for _ in itertools.repeat(None, args.retraining_steps):
                ...
                for _ in itertools.repeat(None, args.n_minibatches):
                    ...
                    try:
                        ...
                        RCNN_loss_weather, RCNN_loss_weather_adv, RCNN_acc_weather = FPN(im_data, im_info, meta_data, gt_boxes, num_boxes, run_partial=True)
                    except Exception:
                        sys.exc_clear()
                        print(im_data)
                        print(im_info)
        ...
        if args.use_adversarial_loss:
            ...
            for _ in itertools.repeat(None, args.n_minibatches):
                ...
                try:
                    ...
                    rois_label = FPN(im_data, im_info, meta_data, gt_boxes, num_boxes, run_partial=False)
                except Exception:
                    sys.exc_clear()
                    print(im_data)
                    print(im_info)
        ...
        if args.eval_display:
            if niter % args.disp_interval == 0:
                ...
                if args.use_tfboard:
                    ...
                    logger.add_scalars(
                        "logs_altitude={}_angle={}_weather=_{}/losses_train".format(args.gamma_altitude, args.gamma_angle, args.gamma_weather, isRestarting(args.use_restarting)), info, niter)
        ...
        if niter % args.save_iters == 0:
            save_name = os.path.join(model_dir, 'fpn_{}_{}_{}_adv.pth'.format(args.session,
